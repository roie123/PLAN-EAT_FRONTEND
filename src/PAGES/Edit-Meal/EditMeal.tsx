        ...
        <>
            {(displaySelection===1)? ( <>
                <div className="edit-meal-cont">
                    ...
                    <input type="number" onChange={(event)=> handleChangeInNumberOfEaters(event)} placeholder={selectedMealFromHome.numberOfEaters.toString()} defaultValue={selectedMealFromHome.numberOfEaters}  />
                    ...
                    <div className="radio-cont">
                        <div className="radio-label-cont">
                            ...
                            <input type="radio"  className={'radio-style'} name={'MEALTIME'}  value={MealTime.Breakfast} onChange={()=>setmealTime(MealTime.Breakfast)} />
                        </div>

                        <div className="radio-label-cont">
                            <p>{MealTime.Lunch}</p>
                            <input type="radio"  className={'radio-style'} name={'MEALTIME'} value={MealTime.Lunch}  onChange={()=>setmealTime(MealTime.Lunch)}/>
                        </div>


                        <div className="radio-label-cont">
                            <p>{MealTime.Dinner}</p>
                            <input type="radio" className={'radio-style'} name={'MEALTIME'} value={MealTime.Dinner}  onChange={()=>setmealTime(MealTime.Dinner)} />
                        </div>
                    </div>

                    <div className="add-and-remove-cont">
                        <button className={'button-5'} onClick={()=> handleClickOnAddRecipes()}>Add Recipes</button>
                        <button className={'button-5'} onClick={()=>setshowingRemoveFeature(true)}>Remove Recipes</button>
                    ...
                    <div className="recipe-list-cont">
                        {displayedRecipes.map((recipe) => (
                            <div key={recipe.id} className="recipe-tag" >
                                ...
                                <p>{recipe.name}</p>
                                {(showingRemoveFeature) ?<button className={'useful-but'} onClick={()=> handleRemoveRecipeFeature(recipe)} >
